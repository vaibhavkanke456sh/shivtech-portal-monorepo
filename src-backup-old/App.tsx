...
function App() {
  ...
  const dashboardStats = {
    ...
    mobileBalances: {
      ...
      phonepe: 0,
      googlePay: 0,
      airtel: mobileBalances.airtel,
      jio: mobileBalances.jio,
      bsnl: mobileBalances.bsnl,
      vodafone: mobileBalances.vodafone
    },
    bankBalances,
    today: {
      tasks: 0,
      ...
      try {
        ...
        if (role) {
          ...
          try {
            ...
            const [clientsRes, tasksRes, fundRes, servicesRes, salesRes] = await Promise.all([
            ...
            if (fundJson?.success) {
              ...
              setDashboardEntries(prev => [...prev, ...mappedEntries]);
            }
            if (salesJson?.success && Array.isArray(salesJson.data)) {
              const salesEntries = salesJson.data as any[];
              setDashboardEntries(prev => [...prev, ...salesEntries]);
            }
          } catch {}
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const salesJson = await salesRes.json().catch(() => ({} as any));
        const mapClient = (c: any): Client => ({ id: c._id, name: c.name, phone: c.phone || '', createdAt: new Date(c.createdAt).toISOString().split('T')[0] });
        const mapTask = (t: any): Task => ({
          id: t._id,
          serialNo: t.serialNo || '',
